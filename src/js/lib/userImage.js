...
export default function userImage() {
    ...
    if (
        urlParam.get("username") == user.username ||
        location.href.includes("change") ||
        location.href.includes("delete")
    ) {
        $user_image.src = "../../resources/images/user_profile_menu.png";
    } else {
        $user_image.src =
            user.img == ""
                ? "../resources/images/default_user_image.png"
                : user.img;
    }
    ...
    document.body.addEventListener("click", (e) => {
        if (
            e.target != $menu &&
            e.target != $user_image &&
            Array.from(document.body.children).includes($menu)
        ) {
            $menu.remove();
        }
    });
